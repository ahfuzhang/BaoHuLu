...
func (g *Generator) RenderGoURLValuesFiles(outDir, baseFileName string) error {
	...
	needsUrlValues := g.urlValuesNeedsSet()
	for _, mt := range msgs {
		// Generated for @UrlValues-annotated messages and every message type they
		// transitively reference, since ToURLValues/FromURLValues call the same
		// methods recursively on nested message fields.
...
// urlValuesNeedsSet computes the transitive closure of message names that require
// URL-values code generated. It starts with messages explicitly annotated with
// @UrlValues, then expands to every message type reachable through a direct or
// repeated message field, because ToURLValues / FromURLValues call the same
// methods on those embedded message types. Map message values are excluded: the
// generated code serializes them via ToJSON, not ToURLValues/FromURLValues.
func (g *Generator) urlValuesNeedsSet() map[string]bool {
	...
	for changed := true; changed; {
		...
		for name := range needs {
			for _, fd := range g.Messages[name].Fields {
				...
				// ToURLValues/FromURLValues; map values use ToJSON.
				if fd.Map || !fd.IsMsg {
					continue
				}
				ref := fd.Type
