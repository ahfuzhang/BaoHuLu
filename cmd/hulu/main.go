...
func runTu(args []string) {
	...
	// Warn about message types referenced but not defined in this proto file.
	// They are assumed to have been generated by a separate run; the generated
	// code calls their methods directly rather than emitting an empty type.
	for _, ref := range pg.UndefinedMessageRefs() {
		fmt.Fprintf(os.Stderr,
			"warning: message %q field %q references undefined message %q; assuming it was generated separately\n",
			ref.Message, ref.Field, ref.Type)
	}
