...
type Generator struct {
	...
	// DeprecatedMessages holds the names of messages that were skipped during
	// collection because they carried a @Deprecated annotation. Fields that
	// reference these messages are removed from generated code, whereas fields
	// that reference messages simply not defined in this proto file (e.g.
	// generated by a previous run) are kept — see UndefinedMessageRefs.
	DeprecatedMessages map[string]bool
}
...
func NewGenerator(pkg string) *Generator {
	return &Generator{
		...
		DeprecatedMessages: make(map[string]bool),
	}
}
...
// removeDeprecatedMsgTypeFields removes fields whose referenced message type was
// skipped during collection because it carried a @Deprecated annotation. Handles
// plain/repeated message fields and map fields whose value type is a deprecated
// message.
//
// Fields that reference a message simply not defined in this proto file (e.g. a
// sub-message generated by a previous run) are deliberately kept: the generated
// code calls that type's methods, assuming it exists elsewhere in the package.
// Such references are reported via UndefinedMessageRefs instead of being dropped.
func (g *Generator) removeDeprecatedMsgTypeFields() {
	for _, md := range g.Messages {
		kept := md.Fields[:0]
		for _, fd := range md.Fields {
			if fd.IsMsg && !fd.Map && g.DeprecatedMessages[fd.Type] {
				continue
			}
			if fd.Map && fd.MapVal != "" && g.DeprecatedMessages[fd.MapVal] {
				continue
			}
			kept = append(kept, fd)
		}
...
// UndefinedMsgRef describes a field that references a message type which is
// neither defined in this proto file nor a known scalar/enum. Such a type is
// assumed to have been generated by a separate run; the generated code calls
// its methods directly rather than fabricating an empty type.
type UndefinedMsgRef struct {
	Message string // name of the message containing the field
	Field   string // field name referencing the undefined type
	Type    string // the undefined message type
}

// UndefinedMessageRefs scans every collected message and returns each field that
// references a message type absent from g.Messages (and not a scalar or enum),
// excluding types intentionally dropped via @Deprecated. Results are returned in
// a stable order (by message name, then field order) so warnings are deterministic.
func (g *Generator) UndefinedMessageRefs() []UndefinedMsgRef {
	isUndefined := func(t string) bool {
		if t == "" {
			return false
		}
		if _, ok := ScalarProtoToGo[t]; ok {
			return false
		}
		if _, ok := g.Enums[t]; ok {
			return false
		}
		if _, ok := g.Messages[t]; ok {
			return false
		}
		if g.DeprecatedMessages[t] {
			return false
		}
		return true
	}
	...
	for _, name := range names {
		...
		for _, fd := range md.Fields {
			...
			if isUndefined(ref) {
				refs = append(refs, UndefinedMsgRef{Message: name, Field: fd.Name, Type: ref})
			}
		}
	}
	return refs
...
func (g *Generator) CollectMessage(m *proto.Message) {
	...
	if msgExt.Deprecated {
		g.DeprecatedMessages[m.Name] = true
